...
const commandListen = require('./util/commandListen')
...
module.exports = class AMPC {
    ...
    async run() {
        ...
        await commandListen(this.fileConfig.commands)
        if (this.fileConfig.localConfig && this.fileConfig.localConfig.targetPath){
            log(chalk.bold('begin the inspection of the target file source'))
    ...
    targetPathCheck({ srcPath, targetPath, backupsFlag }) {
        if (targetPath) {
            fs.access(targetPath, fs.constants.F_OK, async (err) => {
                if (err) {
                    log(`Execute file creation: ${targetPath}`)
